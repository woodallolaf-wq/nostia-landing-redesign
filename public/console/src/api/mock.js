...
/**
 * The same disclosure the real backend attaches to any figure derived from a
 * seeded run (`org_adventure_runs.is_seed`). Identical shape on purpose: the
 * console renders one banner and cannot tell which backend produced it, which is
 * the only way the warning survives a screenshot taken from either.
 */
const SYNTHETIC = {
  contains_seeded_data: true,
  warning: 'Figures include runs generated by a seeding script. Not traction. '
    + 'Do not report these numbers as measured usage.',
};
...
export class MockBackend {
  constructor() {
    this.signedIn = false;
    this.user = { id: 1, username: 'demo-admin', email: 'demo-user-admin@demo.invalid' };
    this.memberships = [
      // Mirrors the seeded pilot organisation in nostia-pivot exactly — same name,
      // same accent, same adventures, same funnel. The tour and the live demo have
      // to tell one story: a prospect who sees this and then watches the phone must
      // not notice the join.
      { org_id: 1, name: 'Demo University Student Affairs', role: 'owner',
    ...
    this.adventures = {
      1: [
        ...
        // The two stage adventures, split because stops are sequential: an
        // unreachable first stop would block everything behind it.
        { id: 2, title: 'Geofence Check', status: 'published', version: 1,
          description: 'A single stop that is deliberately out of range. Shows the locked state.',
          difficulty: 'easy', estimated_minutes: 5,
          updated_at: '2026-08-01T09:14:00Z' },
        { id: 3, title: 'Demo Route', status: 'published', version: 1,
          description: 'The two stops walked live on stage.',
    ...
    this.steps = {
      ...
      3: [
        ...
        step(302, 2, 'The Rejection',
          ...
          + 'the two answers is the only part of this demo that proves the judge is real.',
      ...
      // editor's "why can't I publish this" path is never seen in the demo.
      4: [
    ...
    this.invites = {
      1: [
        // Scoped to Demo Route — this is the code that goes on the printed QR.
     ...
     *
     * These are the numbers scripts/demo-seed-analytics.js actually produces in
     * nostia-pivot, read back through the real orgAnalyticsService. Deriving them
     * from a formula — as this fixture used to — meant the tour and the live demo
     * drifted apart the moment either changed, and the drift is invisible until a
     * prospect is looking at both.
     *
     * Every count is DISTINCT USERS, which is what the server counts. Whoever
     * verifies stop N is exactly the set that reaches stop N+1.
     */
    this.funnels = {
      1: {
        metrics: {
  ...
  async exportAnalyticsCSV(orgId, adventureId, version) {
    ...
    // Same two carriers the server uses: a footer row padded to the header's width
    // (a short row breaks strict parsers, an extra one does not) and a filename
    // prefix. A CSV is the artifact most likely to be opened in a spreadsheet and
    // screenshotted, so it is the one that most needs the caveat attached.
    rows.push([`"SYNTHETIC DEMO DATA — every figure here is invented. Not traction."`,
      ...new Array(header.length - 1).fill('')].join(','));
    return {
      blob: new Blob([rows.join('\n')], { type: 'text/csv' }),
      filename: `SEEDED-adventure-${adventureId}-v${analytics.adventure.version}.csv`,
    };
  }
   ...
   *
   * The two stage adventures (Geofence Check, Demo Route) are suppressed for the
