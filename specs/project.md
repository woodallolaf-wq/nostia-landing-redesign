...
- **Front end only.** This repo carries the marketing site and nothing else.
  The Orgs owner console, its API layer, the deploy pipeline and the app-store
  association were all removed when this repo was made public — they are neither
  needed for a visual redesign nor appropriate to publish. Do not reintroduce
  them.
- **This repo does not deploy.** The live site publishes from the private
  `olafw666-cpu/nostia-landing-source`. Nothing merged here reaches nostia.io
  until a human ports it across.
...
7. **No backend, and nothing private.** No API clients, no auth, no console, no
   billing or analytics surfaces, no credentials, no customer data, no sales
   collateral. This repo is public. If a change would be embarrassing or
   commercially unwise to publish, it does not belong here.
...
- The component test suite passes with zero console errors.
